"""
GACNN model for learning a PDE using Neural network.
...
if __name__ == "__main__":
    ...
    optimizer_x = optim.SGD(netx.parameters(), lr=0.001, momentum=0.5)
    optimizer_y = optim.SGD(nety.parameters(), lr=0.001, momentum=0.5)
    ...
    for epoch in range(5000):
        ...
        outputs_x = netx(inputs)   # shape 1x400
        outputs_y = nety(inputs)
        #print(outputs.shape)
        #targets = torch.zeros(outputs[1].shape)
        # update LHS of loss for each output unit
        #for i in range(data.shape[0]):
        #    targets[i] = analytic_first_order(data[i])
        loss_x = criterion_x(outputs_x, -1.0/3 * outputs_y)
        loss_y = criterion_y(outputs_y, -3 *outputs_x)
        #loss = criterion(outputs[0] + outputs[1], targets)
        loss_x.backward()
        #loss_y.backward()
        optimizer_x.step()
        optimizer_y.step()
        running_loss_x.append(loss_x.item())
        running_loss_y.append(loss_y.item())
        
        # save fig
        if epoch == 4999 or epoch == 9 or epoch == 99 or epoch == 999 or epoch == 0:
            outputs_x = outputs_x.reshape((K,K)).detach().numpy()
            outputs_y = outputs_y.reshape((K,K)).detach().numpy()
            #print(outputs.shape)
            ...
            for i, x in enumerate(x_space):
                # get initial values based on toy example
                #print(init_first_order(x))
                surface[i][0] = init_first_order(x)
                for j, y in enumerate(y_space):
                    surface2[i][j] = analytic_first_order([x, y])
                    #surface[i][j] = outputs[0,i,j] + outputs[1,i,j]

            # update the other boundary
            for j in range(1, len(y_space)):
                surface[0,j] = surface[0,j-1] + outputs_y[0,j] * 1./K;

            # get rest of GACNN solutions
            for i in range(1, len(x_space)):
                for j in range(1, len(y_space)):
                    surface[i,j] = surface[i-1,j-1] + outputs_x[i,j]*1./K + outputs_y[i,j]*1./K
           ...
           # #plt.show()
           # #fig.savefig("Result_at_epoch{}".format(epoch+1), dpi=600)
           # 
           # # error graph
           # error = surface2 - surface
           # fig = plt.figure()
           # ax = fig.gca(projection='3d')
           # surf3 = ax.plot_surface(X, Y, error, rstride=1, cstride=1, cmap=cm.magma,
           #         linewidth=0, antialiased=False, alpha=0.7)
           # ax.set_xlabel("x2")
           # ax.set_ylabel("x1")
           # ax.set_title("Error at epoch {}".format(epoch+1))
           # ax.view_init(elev=10.,azim=60)
            #fig.savefig("Error_at_epoch{}".format(epoch+1), dpi=600)
    
    # graph loss
    epoch_space = [x for x in range(5000)]
    plt.plot(epoch_space, running_loss_x, 'b-', label="loss of NetX")
    plt.plot(epoch_space, running_loss_y, 'y-', label="loss of NetY")
    plt.xlabel("Epoch")
    plt.ylabel("L1 Loss")
    plt.legend()
    plt.show()
