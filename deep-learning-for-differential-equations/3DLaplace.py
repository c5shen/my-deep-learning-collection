...
# constants
K = 10      # number of division to the domain
domain_x = 1.0  # boundary of x1
domain_y = 1.0  # boundary of x2
domain_z = 1.0  # boundary of x3
...
decay = 0           # weight decay for regularization
...
def f(x):
    return -3 * np.sin(x[0] * np.pi) * np.sin(x[1] * np.pi) * np.sin(x[2] * np.pi) * np.pi**2
...
def highdim_result(nx, ny, nz, x_space, y_space, z_space, W, cur_epoch):
    surface = np.zeros((ny, nx, nz))
    surface2 = np.zeros((ny, nx, nz))

    # output analytical solution
    for i, x in enumerate(x_space):
        for j, y in enumerate(y_space):
            for k, z in enumerate(z_space):
                surface[i][j][k] = analytic_solution([x, y, z])

    # output trial solution
    for i, x in enumerate(x_space):
        for j, y in enumerate(y_space):
            for k, z in enumerate(z_space):
                net_outt = neural_network(W, [x, y, z], 0)
                net_outt = net_outt[len(net_outt) - 1]
                surface2[i][j][k] = psy_trial([x, y, z], net_outt)

    # error
    errors = surface2 - surface
    errors = numpy.array(errors)
    return numpy.linalg.norm(errors)
...
def plot_error(error):
    ...
    training, = plt.plot(X, error, color='r', label = "L2 norm error")
    plt.xlabel('Epochs')
    plt.ylabel('error')
    plt.legend()
    plt.show()
...
def plot_loss(trainE, testE):
    fig = plt.figure()
    X = [i for i in range(len(trainE))]
    training, = plt.plot(X, trainE, color='r', label = "Training loss")
    validation, = plt.plot(X, testE, color='b', label = "Test loss")
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
...
def psy_trial(x, net_out):
    # Laplace
    return A(x) + x[0] * (1 - x[0]) * x[1] * (1 - x[1]) * x[2] * (1 - x[2]) * net_out
...
def loss_function(W, pairs):
    ...
    for i in range(0, pairs.shape[0]):
        ...
        err_sqr = ((gradient_of_trial_d2x + gradient_of_trial_d2y + gradient_of_trial_d2z) - func)**2
        loss_sum += err_sqr
...
if __name__ == "__main__":
    if '-dim' in sys.argv:  # input dimension
        input_dim = int(sys.argv[sys.argv.index('-dim') + 1])
    else:
        input_dim = 2
    ...
    if '-d' in sys.argv:
        decay = int(sys.argv[sys.argv.index('-d') + 1])
    ...
    test_z_space = np.linspace(0, domain_z, test_nz)
    x_space = np.linspace(0, domain_x, nx)
    y_space = np.linspace(0, domain_y, ny)
    z_space = np.linspace(0, domain_z, nz)
    ...
    for xi in x_space:
        for yi in y_space:
            for zi in z_space:
                #print(xi,yi)
                if data.shape[0] == 0:
                    data = np.array([xi, yi, zi])
                else:
                    data = np.vstack((data,[xi, yi, zi]))
    # generate test data
    test_data = np.array([])
    for xi in test_x_space:
        for yi in test_y_space:
            for zi in test_z_space:
                if test_data.shape[0] == 0:
                    test_data = np.array([xi, yi, zi])
                else:
                    test_data = np.vstack((test_data, [xi, yi, zi]))
    ...
    for i in range(EPOCH):
        ...
        err = highdim_result(nx, ny, nz, x_space, y_space, z_space, W, i)
        print("cur error: " + str(err))
        error.append(err)
        ...
        for b in range(0, data.shape[0], batch_size):
            ...
            if (len(previous_grad) != 0):
                for t in range(len(loss_grad)):
                    for j in range(len(loss_grad[t])):
                        loss_grad[t][j] = (1 - momentum) * loss_grad[t][j] + momentum * previous_grad[t][j]


            # copy grad into previous grad
            previous_grad = list(map(list, loss_grad))
        ...
        print("takes {0:.2f} seconds".format(duration))
