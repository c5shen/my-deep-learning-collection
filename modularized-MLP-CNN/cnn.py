...
# im2col operation
#start = time.time()
#pad = np.zeros((train_data.shape[0], imgsize+2*padding, imgsize+2*padding,
#                    train_data.shape[3]))
#pad[:,padding:padding+imgsize,padding:padding+imgsize,:] = np.copy(train_data)
#im2col_train = np.zeros((pad.shape[0], pad.shape[3]*filtersize**2, imgsize**2))
#for i in range(0, pad.shape[1] - filtersize + 1, stride):
#    for j in range(0, pad.shape[2] - filtersize + 1, stride):
#        #print(i,j)
#        im2col_train[:,:, imgsize*i + j] = pad[:,i:i+filtersize,j:j+filtersize,:].reshape((
#                        pad.shape[0], filtersize**2 * pad.shape[3]))
## used for backprop on conv
#back_train = np.copy(pad)
#
## val im2col
#pad = np.zeros((val_data.shape[0], imgsize+2*padding, imgsize+2*padding,
#                    val_data.shape[3]))
#pad[:,padding:padding+imgsize,padding:padding+imgsize,:] = np.copy(val_data)
#im2col_val = np.zeros((pad.shape[0], pad.shape[3]*filtersize**2, imgsize**2))
#for i in range(0, pad.shape[1] - filtersize + 1, stride):
#    for j in range(0, pad.shape[2] - filtersize + 1, stride):
#        im2col_val[:,:, imgsize*i + j] = pad[:,i:i+filtersize,j:j+filtersize,:].reshape((
#                        pad.shape[0], filtersize**2 * pad.shape[3]))
#
## test im2col
#pad = np.zeros((test_data.shape[0], imgsize+2*padding, imgsize+2*padding,
#                    test_data.shape[3]))
#pad[:,padding:padding+imgsize,padding:padding+imgsize,:] = np.copy(test_data)
#im2col_test = np.zeros((pad.shape[0], pad.shape[3]*filtersize**2, imgsize**2))
#for i in range(0, pad.shape[1] - filtersize + 1, stride):
#    for j in range(0, pad.shape[2] - filtersize + 1, stride):
#        im2col_test[:,:, imgsize*i + j] = pad[:,i:i+filtersize,j:j+filtersize,:].reshape((
#                        pad.shape[0], filtersize**2 * pad.shape[3]))
#
#print("im2col used {} seconds.".format(time.time() - start))
#print (im2col_train.shape, im2col_val.shape,im2col_test.shape)
...
for repeat in range(ROUND):
    ...
    # (included bias term)
    weights = []
    weights.append(getWeights((imgsize//pooling)**2 * features + 1, OUT, seed))
    ...
    if SAVE and not TEST:
        plt.savefig("write_up/q5_a_{}.png".format(repeat + 1))
    else:
        plt.show()
    plt.cla()
    ...
    plt.title("Train/Validation Classification Error Per Epoch (batch size = {}".format(BATCH))
    ...
    #if SAVE and TEST:
    #    for i in range(best_filters.shape[0]):
    #        np.savetxt('weights_{0}_{1}.txt'.format(fname, trial_n), best_filters[i].ravel())
